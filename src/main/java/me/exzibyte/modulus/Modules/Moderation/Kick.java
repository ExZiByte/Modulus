    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        ...
        if(args[0].equalsIgnoreCase(data.PREFIX + "kick")){
            if(args.length < 2) {
                ...
                eb.setFooter("Insufficient Arguments", event.getJDA().getSelfUser().getDefaultAvatarUrl());
                ...
                kicked.setDescription("You've been kicked from: " + event.getGuild().getName() + " for: There was no reason specified");
                kicked.setColor(0xff5555);
                kicked.setFooter(event.getJDA().getSelfUser().getName() + " Kicked", event.getJDA().getSelfUser().getDefaultAvatarUrl());
                ...
                eb.setFooter(event.getJDA().getSelfUser().getName() + " Kick", event.getJDA().getSelfUser().getDefaultAvatarUrl());
                eb.setTimestamp(Instant.now());
