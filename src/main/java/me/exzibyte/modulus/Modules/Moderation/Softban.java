...
// Lines 42 and 71 need special attention in this class
        ...
        if (args[0].equalsIgnoreCase(data.PREFIX + "softban")) {
            ...
            if(event.getMember().hasPermission(Permission.BAN_MEMBERS)){
                if (args.length < 2) {
                    ...
                    Invite inviteLink = event.getGuild().getTextChannelById("REPLACE WITH CHANNEL ID TO GET AN INVITE FROM").createInvite().setMaxUses(1).setMaxAge(null).complete();
                    Member mentioned = event.getMessage().getMentionedMembers().get(0);

                    banned.setDescription("You've been softbanned from: " + event.getGuild().getName()
                            + " for: There was no reason specified\nHere is an invite link to get back on the server: [" + event.getGuild().getName() + " Invite](https://discord.gg/" + inviteLink.toString().replace("Invite(", "").replace(")","") + " \"Invite Link for" + event.getGuild().getName() + "\")");
                    ...
                    eb.setDescription("You've softbanned: " + mentioned.getAsMention() + " for: No reason specified");
                    ...
                    banned.setDescription("You've been banned from: " + event.getGuild().getName() + " for: " + reason + "\nHere is an invite link to get back on the server: [" + event.getGuild().getName().toString() + " Link](https://discord.gg/" + inviteLink.toString().replace("Invite(", "").replace(")","") + " \"Invite Link for" + event.getGuild().getName() + "\")");
                    banned.setColor(0xff5555);
